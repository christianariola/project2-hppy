...
const ReportMain = (props) => {
  ...
  useEffect(function loadData() {
    axios
      .get("https://pluto-hppy.herokuapp.com/dailySurvey")
      .then((res) => {
  ...
  for (let i = 0; i < surveyItem.length; i++) {
    for (let k = 0; k < showEmailArry.length; k++) {
      if (
        surveyItem[i].dailySurvey.employeeEmail !== undefined &&
        surveyItem[i].dailySurvey.employeeEmail === showEmailArry[k].email
      ) {
        let emailObj = {
  ...
  for (let i = 0; i < monthlyItem.length; i++) {
    for (let k = 0; k < showEmailArry.length; k++) {
      if (
        monthlyItem[i].employeeEmail !== undefined &&
        monthlyItem[i].employeeEmail === showEmailArry[k].email
      ) {
        let monthlyEmailObj = {
  ...
  const sortDailySurvey = [...new Set(dailySingle().map(JSON.stringify))].map(
    JSON.parse
  );
            ...
            <tbody>
              {sortDailySurveyByCompany?.map((sortDailySurveyByCompany) => (
                <tr className="report-content" key={surveyItem._id}>
                  ...
                  <td value={sortDailySurveyByCompany.companyName}>
                    {sortDailySurveyByCompany.companyName}
                  </td>

                  <td>
                    <button
                      className="view-btn"
                      value={sortDailySurveyByCompany.surveyDate}
                      onClick={(e) => handleSelectCompany(e.target.value)}
                    >
                      <Link
                        component={RouterLink}
                        to={`/app/reportchart/${sortDailySurveyByCompany.surveyTitle}/${sortDailySurveyByCompany.surveyDate}/${sortDailySurveyByCompany.companyName}`}
                        variant="button"
                        sx={{ my: 1, mx: 1.5 }}
                        underline="none"
                        color="inherit"
              ...
              {sortDailySurvey?.map((sortDailySurvey) => (
                <tr className="report-content" key={surveyItem.surveyId}>
                  ...
                  <td>
                    <button
                      className="view-btn"
                      value={sortDailySurvey.surveyDate}
                    >
                      <Link
                        component={RouterLink}
                        to={`/app/reportchart/${sortDailySurvey.surveyTitle}/${sortDailySurvey.surveyDate}`}
                        variant="button"
                        sx={{ my: 1, mx: 1.5 }}
                        underline="none"
                        color="inherit"
